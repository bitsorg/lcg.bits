...
function Configure() {
  ...
  # niche file from the G4 plugin list (grep-guarded, idempotent; perl for BSD sed).
  if grep -q 'plugins/Geant4Output2EDM4hep_DRC.cpp' "$BITS_CMAKE_SRC/CMakeLists.txt"; then
    perl -i -ne 'print unless m#\./plugins/Geant4Output2EDM4hep_DRC\.cpp#' "$BITS_CMAKE_SRC/CMakeLists.txt"
  fi
