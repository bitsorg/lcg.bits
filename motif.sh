...
function Configure() {
  ...
  #
  # -L$FLEX_ROOT/lib: the wml flex scanners need libfl's main, but AC_PROG_LEX only
  # sets LEXLIB=-lfl if it can link it, and bits' libfl isn't on a default path
  # (otherwise the tool links with no main).
