...
---
#!/bin/bash -e
# Collect PKG_CONFIG_PATH from every bits-managed *_ROOT so cairo.pc's transitive
# Requires: (pixman-1, fontconfig, …) resolve (module prepend_path only adds direct
# deps). The multiarch glob also covers old lib/<arch>/pkgconfig builds.
