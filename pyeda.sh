...
function Prepare() {
  ...
  perl -i -pe 's/\r//' setup.py
  # Add -Wno-incompatible-pointer-types to suppress C99 → C23 pointer errors in espresso.
  perl -i -pe 's/Extension\("pyeda\.boolalg\.espresso", \*\*ESPRESSO\),/Extension("pyeda.boolalg.espresso", **ESPRESSO, extra_compile_args=["-Wno-incompatible-pointer-types"]),/' setup.py
}