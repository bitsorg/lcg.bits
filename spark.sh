...
#!/bin/bash -e
##############################
# Spark ships as a prebuilt binary distribution (bin/, jars/, python/), not a
# pip-installable package, so install the tree verbatim with BinaryRecipe. (PythonRecipe's
# `pip install .` fails: no top-level setup.py; pyspark lives under python/.)
