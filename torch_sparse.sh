...
function MakeInstall() {
  ...
  # torch_sparse builds a C++ extension whose setup.py imports torch. Use
  # --no-build-isolation so pip sees the bits env's torch (else "No module named 'torch'"),
  # and add each dep's site-packages (torch, scipy, sympy, numpy) to PYTHONPATH ($*_ROOT isn't enough).
  local _r _sp
